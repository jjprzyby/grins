...
namespace GRINS
{
		...
		LinearElastic::LinearElastic( const PhysicsName& physics_name, const GetPot& input) :
		...
		void LinearElastic::mass_residual(bool compute_jacobian,AssemblyContext& context )		
		{
		...
		void LinearElastic::element_time_derivative(bool compute_jacobian, AssemblyContext & context)
			{
		...
		libMesh::Real LinearElastic::C(unsigned int i, unsigned int j, unsigned int k, unsigned int l)
		{
			...
			const libMesh::Real lambda_1 = (E*nu)/((1.+nu)*(1.-2.*nu));
			const libMesh::Real lambda_2 = E/(2.*(1.+nu));
